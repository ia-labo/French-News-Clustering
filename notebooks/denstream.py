...
class MicroCluster():
    ...
    def __init__(self, currenttimestamp, lamb, clusterNumber):
        ...
        self.lastEditTimeStamp = currenttimestamp
        self.lamb = lamb
    ...
    def insertSample(self, sample, timestamp=0):
        ...
        if self.dimensions == None:
            if isinstance(sample.value, list) or isinstance(sample.value, np.ndarray):
...
class OutlierDenStream():
    ...
    def __init__(self, lamb, epsilon=1, minPts=1, beta=1, mu=1, numberInitialSamples=None, startingBuffer=None, tp=60, radiusFactor = 1):
        self.lamb = lamb
        self.minPts = minPts
        self.beta = beta
    ...
    def initWithoutDBScan(self):
        ...
        for sampleNumber in range(0, len(self.buffer)):
            sample = Sample(self.buffer[sampleNumber].tolist(), sampleNumber)
    ...
    def initDBScan(self):
        ...
        for x in range(1, labels[0].max()+1):
            samples = self.buffer[labels[labels[0]==x].index.values]
            sample = Sample(samples.iloc[0], 0)
            sample.setTimestamp(1)

            mc = MicroCluster(1, self.lamb, self.pMicroCluster.N + 1)

            for sampleNumber in range(0, len(samples)):
                sample = Sample(samples.iloc[sampleNumber].tolist(), sampleNumber)
                sample.setTimestamp(sampleNumber+1)
                mc.insertSample(sample, self.currentTimestamp)
